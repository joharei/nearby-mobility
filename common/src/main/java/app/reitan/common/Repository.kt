...
class Repository internal constructor(
    ...
    val scooters = visibleRegion
        .filterNotNull()
        .transformLatest { visibleRegion ->
            do {
                val scooters = fetchScooters(enturApi, visibleRegion)
                emit(scooters)
                delay(1.minutes)
            } while (true)
    ...
    private suspend fun fetchScooters(
        ...
        val radiusKm =
            distance(LatLon(centerLat, centerLon), visibleRegion.southWest).coerceAtMost(1000.0)
